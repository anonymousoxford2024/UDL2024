...
from continual_learning.vcl.vcl_models import VCLSingeHeadNN


def run_vcl_single_head(
    ...
    for task_idx in range(len(tasks)):
        ...
        model = model.train_model(
            train_dl, val_dl, n_epochs, lr=lr, weight_decay=weight_decay
        )
        ...
        print(f"For task {task_idx}: test_acc = {test_acc}")
        model.update_priors()
...
if __name__ == "__main__":
    ...
    for div in ["JS", "KL"]:
        for lr in [1e-4, 1e-5]:
            for weight_decay in [0.0, 1e-5]:
                ...
                model_name = f"VCL {div} Singlehead {ds} lr={lr} wd={weight_decay}"
    ...
    plot_test_accuracies(
        model2accs,
        title=f"Hyperparameter search: VCL {div} Singlehead models on the {ds} test set",
        save_to=f"VCL_{div}_Singlehead_on_{ds}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.png",
    )
